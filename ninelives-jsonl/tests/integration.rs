...
async fn writes_json_lines() {
    ...
    let delay_ms = 50;
    let event =
        PolicyEvent::Retry(RetryEvent::Attempt { attempt, delay: Duration::from_millis(delay_ms) });
    ...
    let val: serde_json::Value =
        serde_json::from_str(first_line).expect("failed to parse JSON from temp file");
