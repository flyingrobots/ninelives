...
## Definition of Done
- [x] `TransportRouter` implemented in [`src/control/transport.rs`](src/control/transport.rs).
- [x] Logic flows: Transport req → decode → CommandRouter → encode response.

## Test Plan
- [x] Mock transport test: verify `TransportRouter` correctly marshals data in and out of the inner `CommandRouter`.
