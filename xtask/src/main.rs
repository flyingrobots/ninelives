...
fn usage() {
  ...
  cargo run --bin tasks it-etcd              # spin up etcd via docker compose and run integration tests
  cargo run --bin tasks it-elastic           # spin up Elasticsearch via docker compose and run integration tests
  cargo run --bin tasks it-otlp              # spin up OTLP collector via docker compose and run integration tests"
    );
}
...
fn cmd_it_etcd() -> Result<()> {
    let env_var = "NINE_LIVES_TEST_ETCD_ENDPOINT";
    let compose_dir = Path::new("ninelives-etcd");

    let provided = std::env::var(env_var).ok();
    let url = provided.clone().unwrap_or_else(|| "http://127.0.0.1:2379".to_string());
    let host_port = parse_host_port(&url).unwrap_or_else(|| "127.0.0.1:2379".to_string());
    ...
    wait_for_host(&host_port, 40, std::time::Duration::from_millis(300))
...
fn cmd_it_otlp() -> Result<()> {
    let env_var = "NINE_LIVES_TEST_OTLP_ENDPOINT";
    let compose_dir = Path::new("ninelives-otlp");

    let provided = std::env::var(env_var).ok();
    let url = provided.clone().unwrap_or_else(|| "http://127.0.0.1:4318".to_string());
    let host_port = parse_host_port(&url).unwrap_or_else(|| "127.0.0.1:4318".to_string());
    ...
    wait_for_host(&host_port, 50, std::time::Duration::from_millis(300))
        .with_context(|| format!("waiting for OTLP collector at {}", host_port))?;

    let status = Command::new("cargo")
        .args(["test", "-p", "ninelives-otlp"])
        .env(env_var, &url)
        .status()
        .context("running cargo test -p ninelives-otlp")?;
    if !status.success() {
        return Err(anyhow!("tests failed"));
    }
    Ok(())
